<div class="overflow-x-auto overflow-y-auto max-h-screen border rounded-lg">
    ...
    <tbody>
        @foreach ($casosDeUso as $caso)
            @php $modulo = str_replace('Gestionar ', '', $caso); @endphp
                ...
                @foreach (['Agregar', 'Editar', 'Eliminar', 'Ver'] as $accion)
                    ...
                    <td class="text-center py-2 px-4">
                        @if (isset($soloLectura) && $soloLectura)
                            @if ($tienePermiso)
                                <svg class="inline-block w-5 h-5 text-green-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
